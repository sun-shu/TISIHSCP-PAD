	...
	return (
		<>
			{data?.map(({
										...
										recordMainId,
										recordTime,
									}: CustomerComposeResultResDTO, index) => (
				<div className=" p-5 bg-white rounded-[20px] flex-col justify-start items-center inline-flex  w-full"
						 key={index}>
					...
					<div className="text-right pt-[10px] w-full">{dayjs(recordTime)?.format('YYYY/MM/DD HH:mm:ss')}
					</div>
				</div>
			))}
		</>
